...
def InitLog():
    ...
    fh = logging.FileHandler("cmd-server.log")
    ...
    while True:
        ...
        for fd, events in epoll_list:
            ...
            if fd == listen_fd.fileno():
                ...
                # 有 可写 事件激活
                sendLen = 0
                result = Runcmd(datalist[fd])
                # 通过 while 循环确保将 buf 中的数据全部发送出去
                while True:
                    # 将之前收到的数据发回 client -- 通过 sendLen 来控制发送位置
                    sendLen += connections[fd].send(result[sendLen:])
                    # 在全部发送完毕后退出 while 循环
                    if sendLen == len(result):
                        break
